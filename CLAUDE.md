...
- `bun run build`, `bun run pack <folder>`, `bun run pack:test`, `bun run upgrade:check [v<previous>]`, `bun run version <x.y.z>`: the publish build, the six tarballs, the packing test (CI's `pack` job), the upgrade check (CI's `upgrade` job: the previous release's addition example moved to the tarballs, then generate, review, tsc and its tests, printing what the upgrade rewrote) and the shared version. A release is a `v<version>` tag; `docs/releasing.md` has the steps and the gate before them (D35).
...
- Relative imports use explicit `.ts` extensions, and no constructor parameter properties, enums or namespaces (Node's type stripping needs both; `erasableSyntaxOnly` in `tsconfig.base.json` enforces the second).
