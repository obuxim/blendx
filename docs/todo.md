...
- [ ] `@blendx/react` misses includes when it invalidates: a mutation invalidates its own table's queries (N.2, written when replies held only their own rows), but since D28 a query with `?include=` holds rows of another table, so `orders.index.queryOptions({ query: { include: 'user' } })` keeps the old user after `users.update`. Proposed: `client.gen.ts` records which tables include which, and a write also invalidates the queries of the tables that include its own. Fix before optimistic updates, which would share the gap. (Found while discussing more for the React adapter; after P16.10.)
- [ ] Infinite scroll on index in `@blendx/react`: `index.infiniteQueryOptions(input)`, the next page derived from `meta` (`page + 1` while `page * per_page < total`), with `page` left out of its input. Its key must differ from the plain query's, since TanStack says not to share one between them, but keeps the `[table]` prefix so mutations invalidate it. The guide also shows numbered pages with `placeholderData: keepPreviousData`, which works today. Offset paging repeats a row in a newest-first list when one is inserted between pages, and skips one when a row is removed; cursor paging would fix that, but it changes the server's contract, so it would be a P16 candidate of its own. (Found while discussing more for the React adapter; after P16.10.)
- [ ] Optimistic updates in `@blendx/react`, decided first (D30), store included. Proposed while discussing: opt-in per mutation; update merges its `json` into every cached copy of the row (its show, and the index pages holding it), destroy removes them, a custom member action takes `optimistic: (row, input) => row`, and store inserts a row with a temporary id. To decide for store: which cached lists get the row (their filters, sort, scope and page), and what it holds for what the server fills (id, defaults, calculate, timestamps, hidden columns). Also proposed: it runs inside the mutation function, as invalidation does, so an app still spreads its own options; a failure invalidates instead of restoring a snapshot, which a retry or a second mutation would make wrong; `client.gen.ts` records each table's primary key column, which need not be `id`. (Found while discussing more for the React adapter; after P16.10.)
