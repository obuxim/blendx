...
- [ ] P15.1 App hooks type `auth` from the app's own `auth` function, not through `Register`: going through `Register` makes an app-level authorize hook that reads `auth` a circular type (TS2502). Done: a type test in the Register project, with an app authorize hook that reads `auth`, passes tsc; the guide shows app hooks reading the identity.
- [ ] P15.2 SQLSTATE 22007 (invalid datetime format) and 22008 (datetime field overflow) answer 422 with a pointer, instead of 500 (D23). Done: an engine test, `packages/spec/errors.md`, and a conformance case.
