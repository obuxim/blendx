...
- [ ] P16.5 The outbox table from the hooks (D27): `blendx generate` writes `outbox.gen.ts`, which re-exports the table once any later hook exists and nothing otherwise, and drizzle.config.gen.ts lists it. Done: goldens, and `blendx migrate generate` creating the table for a fixture with a later hook.
