...
- [ ] P16.8 `?include=` on index and show (D28): the derived rules accept `include` (a new derivation rule), and the engine loads each relation's rows in one query, passes each through the target's show authorization, removes its hidden columns and nests it, or `null`; the reply types carry the optional, nullable fields. Done: engine tests (authorization per row, soft-deleted and missing targets, hidden columns, one query per relation, unknown names answering 422), the rule's test and spec entry, reply type tests.
