...
- [ ] P16.4 The outbox worker (D27): `drainOutbox()` and `startOutbox()` claim due entries with `FOR UPDATE SKIP LOCKED` and a lease, run each level's hook, delete an entry that succeeds, retry one that fails after a growing delay, keep it as failed after the last attempt, and report failures to `onError`; `routes.gen.ts` exports the app's `resources`. Done: worker tests on PGlite, and a real-PostgreSQL test that two workers never run one entry twice.
