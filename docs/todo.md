...
- [ ] P16.11 Has-many includes declared (D31): `includes: { notes: order_notes }` names the rows of another table whose foreign key points at this one, or `{ blend, by, limit, sort }` when the target points at it more than once or the include needs its bound and order; `limit` is required on every has-many include. The types and `blend()` check that the target's model has exactly one single-column foreign key to this table, or that `by` names one; that the name is neither a column nor a belongs-to relation of the table; that the target exposes show without a load hook; that `sort` is a column of the target. Done: type tests and blend() tests, the belongs-to checks unchanged.
