...
- [ ] P16.9 Includes in OpenAPI, the review, conformance and the docs: the `include` parameter and the nested schemas, the review's includes, conformance cases on the shop fixture, the guide (blends, the HTTP API), a cookbook pattern and the CLAUDE.md templates. Done: OpenAPI and review tests, the conformance cases on every runtime, and every doc link resolving.
