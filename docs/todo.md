...
- [ ] N.2 Invalidation: a mutation invalidates every query of its table, and can name other tables its hooks write. Decide whether `createBlendxClient` takes the QueryClient, or reads it from the mutation's context (`onSuccess` receives TanStack's `MutationFunctionContext`).
