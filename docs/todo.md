...
- [ ] Queries ignore TanStack's abort signal: the queryFn calls hc without the query's `signal`, so a cancelled query's request still runs to the end. Check how hc merges a call's `init` with the client's own before passing it. (Found in N.1b.)
