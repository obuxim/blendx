...
- [ ] N.6 A guide page for `@blendx/react` in `docs/guide/`: creating the client, queries and mutations, keys and invalidation, errors and `fieldErrors`, walking through `examples/addition/web`. Done: every snippet comes from the example or a test, and the site builds with every link resolved.
