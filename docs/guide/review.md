...
- Each action has its `route`, the `input` it accepts (one line per field, from the resolved rules), and one line per pipeline stage saying what the default does.
