...
| `deleted_at timestamp`, nullable | Soft delete. Destroy sets it instead of deleting the row, index and show skip such rows, and the blend may list `a.restore()`. |
...
Dates and timestamps are not checked by default: PostgreSQL reads the string, so it accepts words such as `yesterday` and refuses `2026-02-30` with an error blendx does not yet map (a 500). Where a date matters, write the rule yourself, as the tutorial does with `spent_on: z.iso.date()`.
