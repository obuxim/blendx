...
`?include=` also nests has-many relations: for each name, every row of the reply carries the live rows of another table whose foreign key points at it, as an array, never null. Nothing in the DBML names the inverse of a foreign key, so the blend names it: `includes: { notes: order_notes }`, and the target's model must have exactly one single-column foreign key to this table. When it has more than one, as expenses has with `user_id` and `approved_by_id`, the blend says which: `{ submitted: { blend: expenses, by: 'user_id' } }`. The name must not be a column of the table nor one of its belongs-to relations. Every has-many include declares its bound: `{ blend: order_notes, limit: 10 }`, and may declare its order, `sort: '-created_at'`, else the target's primary key ascending. The rows of one relation are loaded in one query for the whole reply, at most `limit` per parent row, by a window function over the foreign key. Each row goes through the target's show as the belongs-to rows do: its policy and authorize hooks, hidden columns removed, live rows only, no show load hook allowed. A row show refuses is dropped, not nulled. The reply types, OpenAPI and the review carry each as an array of the target's public record; the review names its limit and order. Nesting (`user.team`) stays out.
