...
## D35: Publishing to npm (2026-09-14)
All six packages are published: `blendx`, `@blendx/core`, `@blendx/hono`, `@blendx/dbml`, `@blendx/cli` and `@blendx/react`. The facade depends on core and hono, the CLI on dbml, core and the facade, and the React client on the facade, so none can stay private; an app still installs only `blendx`, `@blendx/cli` as a dev dependency (D18) and `@blendx/react` when it has a React page. A published package holds compiled JavaScript and declaration files in `dist/`, emitted by tsc with relative import extensions rewritten from `.ts` to `.js`, and nothing else of the sources; its `exports` and the CLI's `bin` point there. The repository keeps running its tests and typecheck on the sources. All six share one version, released together, 0.1.0 first, and a workspace dependency becomes that exact version when the package is packed. Dependencies stay as they are: hono, drizzle-orm and zod as exact pins inside `blendx` (D5), the database drivers as optional peers, react and TanStack Query as peers of `@blendx/react`. A release is a git tag `v<version>` pushed to GitHub: a workflow runs `bun run check`, builds, checks that the tag names the version in every package, packs each package with Bun, and publishes the tarballs with npm in dependency order (dbml, core, hono, blendx, cli, react), through npm's trusted publishing with provenance, so no token is stored in the repository. A packing test installs the six tarballs into a fresh app and runs it, as a CI job beside `check`.
**Why compiled output:** the sources import `.ts` files and lean on TypeScript's inference; shipping them would make every app typecheck the framework's generics again and need type stripping to run on Node. Declaration files give an app stable, fast types, and the JavaScript runs on Bun and Node alike. **Why one version:** the packages change together, the facade re-exports core, and a mismatch between two of them is a class of bug this rules out; the cost is publishing a package that did not change, which is nothing. **Why a tag and CI:** a release then leaves a record in the repository, runs the whole check first, and needs no credentials on a laptop; trusted publishing ties each publish to this repository's workflow. **Why Bun packs and npm publishes:** `bun pm pack` rewrites `workspace:*` to the version, which npm does not; npm publishes with provenance, which Bun does not.
**Consequences:** P16.18a (the build and the manifests), P16.18b (the packing test and the install docs) and P16.18c (the release workflow).

### D35 note: P16.18a the build and the derived manifest (2026-09-14)
The repository's manifests stay as they are: `exports` on `src/*.ts`, `private: true`, `workspace:*` between packages, so the tests, the typecheck and the examples run on the sources with no build step, and nothing in the repository can be published by accident. `bun run build` emits `dist/` per package with tsc (`tsconfig.build.json`: declarations, `rewriteRelativeImportExtensions`, and `paths` that resolve the sibling packages to their built declarations, so the build runs in dependency order). `bun run pack <folder>` derives each publish manifest from the workspace one: `exports` and `bin` moved to `dist/` with a `types` condition, `files` limited to `dist`, the README and the license, the shared version in place of `workspace:*`, no dev dependencies, `license`, `repository` and `publishConfig.access: public` added; it stages the package in a temporary folder and packs it with `bun pm pack`, and refuses when the six versions differ. Declaration files keep `.ts` in their relative imports, which TypeScript resolves to the `.d.ts` beside them; only the JavaScript is rewritten to `.js`. The CLI reads its version through `createRequire` rather than a JSON import, so `src/` and `dist/` find `../package.json` alike. `check` runs the build, since declaration emit can fail where a plain typecheck passes. The npm side is the maintainer's: the `blendx` name is free, the `@blendx` scope must be created, and trusted publishing is configured per package after its first publish, which npm requires to be done by hand.

### D35 note: the release gate and the upgrade check (2026-09-26)
0.2.0 was tagged on a local check that had hidden four real failures behind Windows path-separator ones, and its API changes (the `migrate()` result, the client map's shape) were found after publishing. `docs/releasing.md` now has a gate before any tag: CI green on GitHub for the commit, the public API diff since the previous tag read, the upgrade check green, and a `docs/changelog.md` entry written from the two. The upgrade check is a script and a CI job (`bun run upgrade:check`, the `upgrade` job, P18.1): `git archive` of the addition example at the previous release tag, its blendx dependencies moved to the freshly packed tarballs by rewriting its own manifest (overrides so the packages find each other there), then `blendx generate`, `blendx review`, `tsc --noEmit` and its tests, printing which generated and review files changed, line endings ignored since a Windows checkout archives CRLF. **Why the previous release's example and not a fixture:** the example is what the guide teaches and what a user's app looks like at that version, generated files and review included, so an upgrade that rewrites them or fails on them is seen as the user would see it. **Why beside the packing test and not inside it:** the packing test proves a fresh install; this proves an upgrade, and a failure should name which.

### D35 note: the workflow stages, the maintainer approves (2026-09-25)
The first release through the workflow, 0.2.0, was refused by the registry with `OIDC permission denied for this action`: npm's trusted publishers now allow `npm stage publish` and treat direct `npm publish` as a separate permission, off by default. The workflow now stages the six tarballs (`npm stage publish`, with provenance, through the same trusted publisher) instead of publishing them, and the maintainer approves each staged version with 2FA, in dependency order, from a logged-in machine or on npmjs.com; nothing is installable until then. The runner installs a pinned npm (12.1.0), since `npm stage` needs 11.15 or later and Node's bundled npm is not pinned. **Why stage rather than tick the box:** a human with a second factor between CI and the registry means a compromised workflow, dependency or runner can stage but never ship, at the cost of six approvals per release, which is nothing for a project with one maintainer. The tag still leaves the record, the check still gates it, and provenance still ties each version to the run.

### D35 note: the first release (2026-09-14)
0.1.0 was published by hand, as D35 planned, and the rehearsal found two faults in the workflow: `npm publish "tarballs/x.tgz"` reads as a GitHub shorthand, so the path is now `./tarballs/`, and the `DRY_RUN` expression gave the string `false` on a tag push, which `${DRY_RUN:+--dry-run}` treats as set, so every tagged release would have been a dry run; it is now `'true'` or empty. The publish step also skips a package already on npm at the version, so a rerun after a partial failure publishes what is missing, and a tag for a hand-published version, `v0.1.0`, runs green and proves the trusted publishing setup.

## D34: PUT replaces the public representation (2026-09-14)
A blend may list `a.replace()`, a built-in member action served as `PUT /<table>/:id` (one segment per key column on a composite key, D33) that answers 200 with the record. `update` stays PATCH. Its rules are the store rules without the key columns: a NOT NULL column without a default is required, the rest may be left out, and an unknown key is refused. What it saves is the body plus a reset of every writable column the body leaves out, except the key columns and the hidden ones: a column with a schema default goes back to that default (SQL `DEFAULT`), and a nullable column without one becomes null. Hidden columns change only when the body carries them, as under PATCH, and the rules leave them optional even where store requires them, since the client never saw them. A row that does not exist is 404, never created: store creates rows, and their identity keys are generated. The pipeline is update's: the row is loaded `FOR UPDATE`, authorize runs, calculate receives the body with the nulls already in `prev`, save writes the result and the defaults, then `after` and `later` run. The review lists the columns a replace resets; OpenAPI has the `put` operation with the store body minus the key columns; the React adapter treats `optimistic: true` on replace as on update, merging the body, and the refetch that follows sets the reset columns.
**Why replace the representation and not the row:** PUT means the resource becomes what the body says. The resource the client sees is the public record, so a hidden column such as a password is not part of what it replaces; resetting it would wipe or fail on a column the client never saw. **Why reset to the default and null:** a missing optional column under PUT means "not set", and the schema already says what an unset column holds. Requiring every column would make PUT unusable on wide tables, and leaving missing columns untouched is PATCH. **Why no key columns in the body:** the path names the row; a body that also named it could move it. PATCH keeps accepting them, as it accepts any non-generated column, since changing that is unrelated to PUT. **Why defaults in save, not in prev:** calculate is pure and typed by the table's insert types, where a SQL `DEFAULT` marker has no place; nulls fit the types, defaults are applied where the row is written, and a column calculate sets is not reset.
**Consequences:** P16.17a (the action, its rules as a derivation rule with its conformance case, the engine and the types) and P16.17b (OpenAPI, the review, the React adapter, the docs).

## D33: A composite primary key is one path segment per column (2026-09-13)
A table whose primary key spans several columns, `(order_id, line) [pk]` in its `indexes`, is served like any other, and its member routes carry one path segment per key column, named by the column, in the key's declared order: `GET /order_items/:order_id/:line`, `PATCH /order_items/:order_id/:line`, `POST /order_items/:order_id/:line/<action>`. A single-column key keeps `:id` whatever the column is called, so no existing route changes. The engine's member load and lock match every key column, and a segment that cannot be its column's type answers 404 as `/orders/abc` does; the default index sort and the has-many tiebreak order by every key column; the row an update or delete saves is found by all of them. Key columns are ordinary input: DR-STORE-GENERATED excludes only an identity column, so store takes them in its body and update takes them as it takes a non-generated single key today. `ModelMeta.primaryKey` becomes the list of key columns, never empty. OpenAPI lists one path parameter per column, each with its column's schema; the review's route lines show the segments; `hc` types the route as `client.order_items[':order_id'][':line']`, with `param: { order_id, line }`. `client.gen.ts` records `key` as a list of `{ column, type }` in the key's order, and the React adapter matches a cached row by every column; an optimistic store keeps the key columns the input carries and gives a temporary value only to one it leaves out, which is only ever a single identity key. Relations stay single-column: a composite-key table can be a has-many target and a belongs-to source through its single-column foreign keys, and a foreign key that spans several columns, which is what pointing at a composite key takes, derives no relation, as today; composite refs in includes are a later candidate.
**Why a segment per column:** a single segment such as `/order_items/1,2` needs an escape for a string key that holds a comma, types the key as one string, and reads as nothing else in the API does. Segments named by column type each part, and the route says what it takes. **Why `:id` stays:** it is what every app, test, guide and `hc` call has, and the name of the single segment carries no information a composite one needs. **Why key columns are input:** a composite key is nearly always made of foreign keys and a discriminator the client knows, so the client supplies it, as it supplies a `code varchar [pk]` today. **Why relations stay single-column:** a foreign key that spans columns names its relation no more than a has-many does, and D28's `_id` convention has nothing to strip; it deserves a decision of its own when an app needs it.
**Consequences:** P16.16a (the model, the schema emitter and validation), P16.16b (routes and the engine), P16.16c (OpenAPI, the review, the client map and the React adapter) and P16.16d (conformance and the docs).
...
### D25 note: P16.15 invalidation follows paths (2026-09-13)
A query that asked for `include: 'notes.author'` holds rows of every table on the path, so the adapter walks each asked path through the `tables` map, `orders` to `order_notes` to `users`, and a write to any table it holds refetches the query; a query that asked for `notes` alone refetches only after a write to `order_notes`. The map keeps its flat shape, one target table per include name, since a path is read segment by segment from it (D32).
