...
**Consequences:** P16.18a (the build and the manifests), P16.18b (the packing test and the install docs) and P16.18c (the release workflow). The npm side is the maintainer's: the `blendx` name is free, the `@blendx` scope must be created, and trusted publishing is configured per package after its first publish, which npm requires to be done by hand.
